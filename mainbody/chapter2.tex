...
按成都理工大学地球物理学院的本科论文格式要求, 本文正文使用的是{\zihao{-4}小四号}，应该来说是不用同学们自行改字号的，如果有什么特殊需求也可以参考此处进行字号调整，只需将字号调节命令与需调节内容放在同一个大括号内即可。
...
\begin{figure}[!htb]
  \begin{minipage}[t]{0.5\linewidth}
    \centering
     ...
     \chartname{我书读得多，不会骗你}\label{fig:1}
  \end{minipage}%
  \begin{minipage}[t]{0.5\linewidth}
    \centering
    \includegraphics[scale=0.5,angle=-45]{images2.jpg}
    \chartname{哈哈哈}\label{fig:2}
  \end{minipage}
\end{figure}
...
\begin{figure}[!htb]
    ...
    \centering
     ...
     \chartname{太难了}\label{fig:3}
  \end{minipage}%
  \begin{minipage}[t]{0.5\linewidth}
    \centering
    \includegraphics[scale=0.6,angle=45]{images4.jpg}
    \chartname{砰砰}\label{fig:4}
  \end{minipage}
\end{figure}
